...
if [ -n "$MEM_MB" ] && [ "$MEM_MB" -lt 2048 ] 2>/dev/null; then
  warn "Colima VM has ${MEM_MB} MB RAM — builds need at least 2 GB (OOM risk)."
  warn "Restart with more memory: colima start -p ${COLIMA_PROFILE} --memory 4"
fi
...
# ── Onboarding wizard ─────────────────────────────────────────────
info "OpenClaw Docker Setup (Colima)"
echo ""
...
# ── Write .env ─────────────────────────────────────────────────────
cat > "$ENV_FILE" <<EOF
# Generated by colima-setup.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
OPENCLAW_DOCKER_SOCKET=${COLIMA_SOCKET}
